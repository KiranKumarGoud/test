Vue.component("list-search", {
  template: `
  <div class="tab-pane bibliography-list-search active" id="m_tabs_1_1" role="tabpanel"> 
                ...
                <div class="col-xl-12"> 
                    ...
                    <div class="row" style="margin: 0px;">
                        <div class="col-5">
                            ...
                            v-model="$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue"
                            @change="selectChange"
                            :options="$store.state.bibliographySearch.listSearch.advanceSearch.selectOptions"
                            >
                            </b-form-select>
                        </div>
                        <div class="col-5"> 
                            <!-- STARTS Multi Select drop down control -->
                            <div v-if="$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue == 'ref_id' && $store.state.bibliographySearch.listSearch.fileName == null">
                            ...
                            <div v-if="$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue != 'ref_id' && $store.state.bibliographySearch.listSearch.fileName == null">
                                <!-- STARTS Multi Select drop down control -->
                                <auto-complete
                                    :selected-value="$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue"
                                    search-type="BIBLIOGRAPHY_ADVANCE_SEARCH"
                                    ...
                                    :disable-auto-complete="($store.state.bibliographySearch.listSearch.advanceSearch.selectedValue != null ) ? false : true"
                                    @auto-complete-selection-change="autoCompleteChange($event)"
                                ></auto-complete>
                            </div>
                            <div class="col-md-6 freeze-field" v-else-if="$store.state.bibliographySearch.listSearch.fileName != null">
                                {{$store.state.bibliographySearch.listSearch.fileName}}
                            </div>
                        ...
                        <div class="col-1" style="width: 5%"> 
                            ...
                            <q-uploader
                                ...
                                accept=".txt"
                                @added="fileAdded"
                                :disable="($store.state.bibliographySearch.listSearch.advanceSearch.selectedValue != null ) ? false : true"
                                ref="bibliographyListSearchUpload"
                        ...
                        <div class="col-1" style="margin-top:-2px;"> 
                            ...
                            <a href="javascript:void(0)" @click="resetAutoComplete()" :disabled="($store.state.bibliographySearch.listSearch.advanceSearch.selectedValue != null ) ? false : true" m-portlet-tool="reload" class="m-portlet__nav-link m-portlet__nav-link--icon"> <i class="la la-refresh refresh-btn-stl" style="font-size: 2.5rem;"></i> </a> 
                        </div>
                    </div>
  ...
  mounted() {
    ...
    if(Object.keys(this.$store.state.simpleSearch.simpleSearchRowData[curItemIndex]['bibliographySearch']['listSearch']).length > 0) {
      this.$store.state.bibliographySearch.listSearch = this.$store.state.simpleSearch.simpleSearchRowData[curItemIndex]['bibliographySearch']['listSearch']
    }else{
      this.$store.state.bibliographySearch = JSON.parse(glb__initialBibliographySearchState)
    }
  },
  data() {
    return {
  ...
  methods: {
    captureRefIds(){
      ...
      this.$store.state.bibliographySearch.listSearch.advanceSearch.refIds = formatedrefIds;
    },
    resetFileUpload() {
      this.$refs.bibliographyListSearchUpload.reset();
    },
    getInitalCounts() {
      this.$store
    ...
    resetAutoComplete() {
      ...
      this.$store.state.bibliographySearch.listSearch.advanceSearch.autoCompleteSelections = []
      this.$store.state.bibliographySearch.listSearch.fileData = []
      this.$store.state.bibliographySearch.listSearch.fileName = null
      this.$store.state.advanceSearch.currentItem.advanceSearch.autoComplete = []
    },
    selectChange(event) {
      this.resetAutoComplete();
      if (event) {
        this.$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue = event;
      }
    },
    resetChanges() {
      this.resetAutoComplete();
      ...
      this.$store.state.simpleSearch.simpleSearchRowData[curItemIndex]['bibliographySearch']['listSearch'] =  JSON.parse(glb__initialBibliographySearchState).listSearch
    ...
    getSelectedValueName(selectedValue) {
      let textName = ''
      this.$store.state.bibliographySearch.listSearch.advanceSearch.selectOptions.forEach(element => {
      ...
      this.$bvModal
        ...
        .then(value => {
          ...
          this.$store.state.simpleSearch.simpleSearchRowData[
            curItemIndex
          ]['isBibliographyAdvanceSearch'] = true

          this.$store.state.simpleSearch.simpleSearchRowData[
            curItemIndex
          ]['bibliographySearch']['listSearch'] = this.$store.state.bibliographySearch.listSearch;

          // make sure to update the rowdata advanceSearch keys for reflecting in the simple search
          if(this.$store.state.bibliographySearch.listSearch.fileData.length == 0) {

            this.$store.state.simpleSearch.simpleSearchRowData[
              curItemIndex
            ]["advanceSearch"]['autoComplete'] = this.$store.state.bibliographySearch.listSearch.advanceSearch.autoCompleteSelections;
           
          }else{
            this.$store.state.simpleSearch.simpleSearchRowData[
              curItemIndex
            ]["fileData"] = this.$store.state.bibliographySearch.listSearch.fileData;
            this.$store.state.simpleSearch.simpleSearchRowData[
              curItemIndex
            ]["fileName"] = this.$store.state.bibliographySearch.listSearch.fileName;
          }

          this.$store.state.simpleSearch.simpleSearchRowData[
            curItemIndex
          ]['advanceSearchSelectedValue'] = this.$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue;
          this.$store.state.simpleSearch.simpleSearchRowData[
            curItemIndex
          ]['advanceSearchSelectedValueName'] = this.getSelectedValueName(this.$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue)
      ...
      this.readFileContent(input[0])
        .then(content => {
          ...
          if (filterFileContents.length > 0) {
            ...
            this.$store.state.simpleSearch.bibliographyListSearchFileUpload = true
           
            
            this.$store.state.bibliographySearch.listSearch.fileName = event[0].name;
            this.$store.state.bibliographySearch.listSearch.fileData = filterFileContents;
          } else {
    ...
    autoCompleteChange(event) {
      if (event.length > 0) {
        this.$store.state.bibliographySearch.listSearch.advanceSearch.autoCompleteSelections = event;
  ...
  computed: {
    disableAdd(){
      if(
        this.$store.state.bibliographySearch.listSearch.advanceSearch.autoCompleteSelections.length > 0 ||
        this.$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue == 'ref_id' ||
        this.$store.state.bibliographySearch.listSearch.fileData.length > 0
        ){
          return false
        }
    ...
    disableClear(){
      if(this.$store.state.bibliographySearch.listSearch.advanceSearch.selectedValue){
          return false
        }
      return true;
    },
