Vue.component("toxicity-treeview-search", {
                                  ...
                                  <div class="col-xl-12"> 
                                      ...
                                      <div class="m-widget14">
                                          <div class="m-widget14__header m--margin-bottom-30">
                                              <h3 class="m-widget14__title"> Toxicity Tree view Input </h3>                                
                                          </div>
                                          <div class="row col-12">
                                              <div class="col-6">
                                                  <auto-complete
                                                      selected-value="ToxicityTreeView"
                                                ...
                                                <li v-for="item in $store.state.treeViewSearch.allSelectedNodes">
                                                  <b>
                                                    <q-chip
                                                      ...
                                                      class="bg-white"
                                                      removable
                                                      @remove="deleteCurrentNode(item.toxicityOntologyId)"
                                                    >
                                                      <div>
  ...
  methods: {
    ...
    deleteCurrentNode(key) {
      this.$store.state.treeViewSearch.allSelectedNodes.forEach((item,index) => {
        if(item.toxicityOntologyId == key) {
    ...
    expandTreeAndHighlightSearchedKey(parentNode, userSearchKey) {
        ...
        this.$refs.treeViewComponent.setExpanded(parentNode.toxicityOntologyId, true)
    },
    searchTreeView(){
      this.postDataParms = []
      ...
      .then(response => {
        if(response.data.length > 0) {
          ...
          response.data.forEach(node => {
            let parentNode = this.$refs.treeViewComponent.getNodeByKey(node.toxicityOntologyId)
            parentNode.children  = node.children
            this.expandTreeAndHighlightSearchedKey(parentNode, this.copyUserSearchValue)
          })

          console.log(this.$store.state.treeViewSearch.selectedNodeIds)
    ...
    lazyLoadItemsEvent({ node, key, done, fail }){
      this.$store.dispatch('treeViewSearch/GET_LAZY_LOAD_TREE_ITEMS', node.toxicityOntologyId)
      .then(response => {
